...
| Gate | Runs | Checks |
|---|---|---|
| `npm run prose` (Vale) | `ci/workflows/docs.yml` (staged) | spelling, Google's conventions, and the banned list, at the levels set in `.vale.ini` |
...
| `ts/scripts/vale-counts.cjs` | `npm run prose` | that every count in `.vale.ini`, and the total below, are what Vale reports |
...
a clean run over the gated set: 84 alerts across 2 files. Those numbers
...
activate. `npm run prose` in `ts/` runs the same check locally
today, and `ts/test/docs.test.js` runs in `npm test` now.
